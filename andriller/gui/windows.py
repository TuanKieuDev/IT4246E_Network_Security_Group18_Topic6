...
class MainWindow(BaseWindow):
    def __init__(self, **kwargs):
        ...
        self.output_button = ttk.Button(
            opframe, text="Output..", command=self.set_output
        )
        self.output_button.pack(side=tk.LEFT)
        createToolTip(
            self.output_button, "Select the output location where data will be saved to"
        )
        ttk.Label(opframe, textvariable=self.OUTPUT, padding=(5, 0)).pack(
            expand=True, fill=tk.X, side=tk.LEFT
        )
        ...
        createToolTip(
            self.extract_button,
            "Extract and decode data from a connected Android device",
        )
        ...
        self.force_backup_button = ttk.Checkbutton(
            extract_adb_frame, text="Use AB method (ignore root)", var=self.force_backup
        )
        self.force_backup_button.grid(row=4, column=0, columnspan=2, sticky=tk.W)
        createToolTip(
            self.force_backup_button,
            "If rooted - force Android Backup extraction instead",
        )

        self.extract_shared = tk.IntVar()
        self.extract_shared_button = ttk.Checkbutton(
            extract_adb_frame, text="Extract Shared Storage", var=self.extract_shared
        )
        self.extract_shared_button.grid(row=5, column=0, columnspan=2, sticky=tk.W)
        createToolTip(
            self.extract_shared_button,
            "File system extraction of shared storage\n(Pictutes, Videos, Audios, other files)",
        )
        ...
        self.extract_folder = ttk.Button(
            extract_folder_frame,
            text="Directory..",
        )
        ...
        createToolTip(
            self.extract_folder,
            "Choose the '/data/data' directory to be parsed and data decoded",
        )
        noteframe.hide(extract_folder_frame)

        # TAR extract controls
        extract_tar_frame = ttk.Frame(noteframe, padding=(5, 0))
        noteframe.add(extract_tar_frame, text="Parse (.TAR)")
        self.extract_tar = ttk.Button(
            extract_tar_frame,
            text="TAR File..",
        )
        ...
        createToolTip(
            self.extract_tar,
            "Choose the 'data.tar' backup file to be parsed and data decoded",
        )
        noteframe.hide(extract_tar_frame)

        # AB extract controls
        extract_backup_frame = ttk.Frame(noteframe, padding=(5, 0))
        noteframe.add(extract_backup_frame, text="Parse (.AB)")
        self.extract_backup = ttk.Button(
            extract_backup_frame,
            text="AB File..",
        )
        ...
        createToolTip(
            self.extract_backup,
            "Choose the 'backup.ab' file to be parsed and data decoded",
        )
        noteframe.hide(extract_backup_frame)
        ...
        # Bottom buttons
        clear_field = ttk.Button(
            self.mainframe, text="Clear Log", command=self.clear_log
        )
    ...
    def build_file_menus(self):
        ...
        menu_file.add_command(
            label="Exit", underline=1, command=self.root.destroy, accelerator="Esc * 2"
        )
    ...
    def build_help_menus(self):
        ...
        if not getattr(sys, "frozen", False):
            menu_help.add_command(
                label="Run Update",
                command=lambda: self.conf.upgrade_package(logger=self.logger),
            )
            menu_help.add_separator()
    ...
    def build_locks_menus(self):
        ...
        menu_locks.add_command(
            label="Gesture Pattern (Legacy)", command=self.brute_pattern
        )
        menu_locks.add_separator()
        menu_locks.add_command(
            label="PIN Cracking (Generic)", command=self.brute_gen_pin
        )
        menu_locks.add_command(
            label="Password by Dictionary (Generic)", command=self.brute_gen_dict
        )
        menu_locks.add_command(
            label="Password by Brute-Force (Generic)", command=self.brute_force_gen
        )
        menu_locks.add_separator()
        menu_locks.add_command(
            label="PIN Cracking (Samsung)", command=self.brute_sam_pin
        )
        menu_locks.add_command(
            label="Password by Dictionary (Samsung)", command=self.brute_sam_dict
        )
        # menu_locks.add_separator()
    ...
    def build_tools_menus(self):
        ...
        menu_tools.add_command(
            label="Extract AB to folder..", command=self.ab_to_folder
        )
    ...
    def check_usb(self, event):
        with disable_control(event):
            ...
            if status == "offline":
                ...
                self.DeviceStatus.set(
                    f"Serial ID: {serial}" if serial else "Device not detected!"
                )
